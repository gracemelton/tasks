...
export function getNonEmptyQuestions(questions: Question[]): Question[] {
    ...
    const noEmpty = questionMap.filter(
...
export function changeQuestionTypeById(
    ...
    const copyArray = questions.map(
        (question: Question): Question => ({ ...question })
    );
    const newType = copyArray.map(
        (question: Question): Question =>
            question.id === targetId
                ? { ...question, type: newQuestionType }
                : { ...question }
    );
    const questionType = newType.map(
        (question: Question): Question =>
            question.id === targetId &&
...
export function editOption(
    ...
    //     newArray[targetIndex].options.splice(-1, 0, newOption);
    // } else {
    //     newArray[targetIndex].options.splice(targetOptionIndex, 1, newOption);
    // }
    const finalArray = newArray.map((question: Question): Question => {
        if (question.id !== targetId) {
            ...
            addOption[targetOptionIndex] = newOption;
            return { ...question, options: addOption };
        }
    });
...
export function duplicateQuestionInArray(
    ...
    const duplicate = duplicateQuestion(newId, newArray[targetIndex]);
    newArray.splice(targetIndex + 1, 0, duplicate);
    return newArray;
}

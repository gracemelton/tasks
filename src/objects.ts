...
export function isCorrect(question: Question, answer: string): boolean {
    const answerCompare = answer.trim().toLowerCase();
    const questionCompare = question.expected.trim().toLowerCase();
    if (answerCompare === questionCompare) {
        return true;
    } else {
        return false;
    }
...
export function toShortForm(question: Question): string {
    const answer = question.id + ": " + question.name.slice(0, 10);
    return answer;
}
